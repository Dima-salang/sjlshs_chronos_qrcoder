...
class QRCodeGenerator:
    ...
    def create_qr_code(self, data: Dict[str, Any], output_path: str) -> bool:
        ...
        try:
            ...
            # Prepare the final data to be encoded
            if self.crypto:
                # If encryption is enabled, data should be a dict or can be converted to one
                # But here we currently pass a string (student_id), so we'll wrap it
                encode_payload = {"id": data} if isinstance(data, str) else data
                qr_data = self.crypto.encrypt_data(encode_payload)
            else:
                # If no encryption, just use the raw data string
                qr_data = str(data)
            ...
            print(f"Error generating QR code at {output_path}: {str(e)}")
            return False
